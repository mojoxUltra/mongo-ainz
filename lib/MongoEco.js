...
*/
  ...
  async Work(UserID, GuildID, time) {
    let UserData = await data.findOne({
      userID: UserID,
      gid: GuildID
    })
    ...
    let amount_rand = [3200, 2312, 780, 912, 1000, 402, 690, 701];
    let AMT = amount_rand[Math.floor(Math.random() * amount_rand.length)];
    let timeout = (time !== "default") ? time : 1000;
    ...
    if (!UserData) {
      let AddUser = new data({
        ...
        gid: GuildID,
        wallet: Number(AMT),
        lastUsedCustom: Date.now()
      });

      await AddUser.save()
      return { "amount": Number(AMT), "newbal": AddUser.wallet };
    }
    let custom = UserData.lastUsedCustom;
    if (custom !== null && timeout - (Date.now() - custom) > 0) {
      return "ALREADY_USED"
    }
    else {
      UserData.wallet += Number(AMT);
      UserData.lastUsedCustom = Date.now()
      await UserData.save()
      return { "amount": Number(AMT), "newbal": UserData.wallet };
    }
  }
  ...
  async Hourly(UserID, GuildID) {
    ...
    if (!UserData) {
      let AddUser = new data({
        ...
        gid: GuildID,
        wallet: Number(AMT),
        lastUsedHourly: Date.now()
      });

      await AddUser.save()
      return { "amount": Number(AMT), "newbal": AddUser.wallet };
    }
    let hourly = UserData.lastUsedHourly;
    if (hourly !== null && timeout - (Date.now() - hourly) > 0) {
      return "ALREADY_USED"
    }
    else {
      UserData.wallet += Number(AMT);
      UserData.lastUsedHourly = Date.now()
      await UserData.save()
      return { "amount": Number(AMT), "newbal": UserData.wallet };
    }
