...
*/
  ...
  async Work(UserID, GuildID, timeout, AMT) {
    let UserData = await data.findOne({
      userID: UserID,
      gid: GuildID
    })
    ...
    let data = [3200, 2312, 780, 912, 1000, 402, 690, 701];
    AMT = (AMT !== "default" ) ? AMT : data[Math.floor(Math.random() * data.length)];
    timeout = (timeout !== "default") ? timeout : 1000;
